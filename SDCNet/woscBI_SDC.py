...
class BI_SDC(nn.Module):
	def __init__(self, args, field=None):
		...
		self.conv1_D = nn.ModuleList(
						[nn.Conv2d(Ci, Co, (1, D)) for _ in range(4)])
#		self.convK_1 = nn.ModuleList(
#						[nn.Conv2d(4*Co, Co, (K, 1), padding=(i,0)) \
#						for i, K in enumerate(Ks)])
		self.convK_1 = nn.ModuleList(
						[nn.Conv2d(4*Co, Co, (K, 1), padding=(i,0)) \
						for i, K in enumerate(Ks)])
		#self.conv3 = nn.Conv2d(Co, Co, (3, 1), padding=(1,0))
		#self.conv4 = nn.Conv2d(Co, 3*Co, (3, 1), padding=(1,0))

		self.conv3 = nn.Conv2d(5*Co, Co, (3, 1), padding=(1,0))
		self.conv4 = nn.Conv2d(6*Co, 3*Co, (3, 1), padding=(1,0))
		self.CNN_dropout = nn.Dropout(args.CNN_dropout)
		self.fc1 = nn.Linear(3*len(Ks)*Co+2*H, C)
	...
	def forward(self, x):
		...
		out = H * S_alpha		# out [batch_size, sen_len, 2*H]

		# x = x.unsqueeze(1)	# x_ [batch_size, 1, sen_len, D]
		xD_out = [conv(x_embed) for conv in self.conv1_D]
		
		#xD_out[i] [batch_size, Co, sen_len, 1]
		xK_in = [torch.cat([x_embed.permute(0,3,2,1), d], 1) for d in xD_out]
								# x[i] [batch_size, Co+2*H, sen_len, 1]	

		xK_out = [F.relu(conv(xK_in[i])) for i, conv in enumerate(self.convK_1)]
#		xK_out = [F.relu(conv(xD_out[i])) for i, conv in enumerate(self.convK_1)]
...
#		x3_out = [F.relu(self.conv3(i)) for i in xK_out]        
		...
		x4 = [F.relu(self.conv4(i)).squeeze(3) for i in x4_in]
#		x4 = [F.relu(self.conv4(i)).squeeze(3) for i in x3_out]
#		x = [torch.add(i.squeeze(3), x_embed.squeeze(1).permute(0,2,1)) \
#			 for i in x4]
#		x = [torch.cat(i.squeeze(3), 1) for i in x4]
        
		x =[F.max_pool1d(i, i.size(2)).squeeze(2) for i in x4]
		x=torch.cat(x,1)        
