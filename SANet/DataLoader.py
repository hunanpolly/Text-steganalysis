...
class DatasetIterater(object):
    ...
    def __next__(self):
        if self.residue and self.index == self.n_batches:#有余数，index=n_batches意味着不完整的这个batch
            batches = self.batches[self.index*self.batch_size:len(self.batches)]
            self.index += 1
            batches = self._to_tensor(batches)
            return batches
        elif not self.residue and self.index == self.n_batches:#没有余数，n_batch就是最后一个，index=n_batches意味着下一个循环
            self.index = 0
        ...
        elif self.index > self.n_batches:#如果没有余数，在上一个循环就处理了，走到这里意味着有余数，index=n_batches+1意味着进入下一个循环
            self.index = 0
            batches = self.batches[
                      self.index * self.batch_size:(self.index + 1) * self.batch_size]
            self.index += 1
